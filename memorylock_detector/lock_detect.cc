...
class LockTracker {
 public:
  ...
  void RecordLockAcquired(pthread_mutex_t* mutex) {
    ...
    if (it != active_locks_.end()) {
      ...
      waiting_locks.erase(std::remove(waiting_locks.begin(), waiting_locks.end(), lock_addr), waiting_locks.end());
    }
  }
 ...
 private:
   ...
   */
  bool DetectDeadlockDFS(void* current_lock,
...
bool LockTracker::DetectDeadlock(void* lock_addr, pthread_t thread_id) {
  ...
  // 使用深度优先搜索检查所有可能的等待路径
  bool found_deadlock = DetectDeadlockDFS(lock_addr, thread_id, visited_threads, lock_chain);
 ...
 * 递归检查锁的依赖关系，寻找形成环路的依赖链
 */
bool LockTracker::DetectDeadlockDFS(void* current_lock,
                                    pthread_t current_thread,
                                    std::unordered_set<pthread_t>& visited_threads,
                                    std::vector<std::pair<void*, pthread_t>>& lock_chain) {
  ...
  for (void* waited_lock : info.waiting_for) {
    ...
    // 递归检查这条路径
    if (DetectDeadlockDFS(waited_lock, owner_thread, visited_threads, lock_chain)) {
...
void LockTracker::PrintLockInfo(const LockInfo& info) const {
  TRACKER_PRINT("Lock %p (Mutex) held by thread %lu\n", info.lock_addr, info.owner_thread);
  TRACKER_PRINT("Acquired at:\n");

  char** symbols = backtrace_symbols(info.callstack.data(), info.callstack.size());
  if (symbols) {
  ...
  if (!info.waiting_for.empty()) {
    ...
    for (void* waited_lock : info.waiting_for) {
      ...
      if (it != active_locks_.end()) {
        TRACKER_PRINT(" %p (held by thread %lu)", waited_lock, it->second.owner_thread);
      } else {
  ...
  if (!thread_info_.empty()) {
    ...
    for (const auto& pair : thread_info_) {
      ...
      for (void* lock : pair.second.waiting_locks) {
        ...
        if (it != active_locks_.end()) {
          TRACKER_PRINT(" %p (held by thread %lu)", lock, it->second.owner_thread);
        } else {
          TRACKER_PRINT(" %p", lock);
        }
      }
...
void LockTracker::PrintCallStack(FILE* file, const std::vector<void*>& callstack) const {
...
void LockHook::Start() {
  ...
  try {
    if (hook_->ReplaceFunction("pthread_mutex_lock", reinterpret_cast<void*>(&HookedPthreadMutexLock), nullptr) !=
        PLTHook::SUCCESS) {
      TRACKER_ERROR("Failed to hook pthread_mutex_lock");
    }

    if (hook_->ReplaceFunction("pthread_mutex_unlock", reinterpret_cast<void*>(&HookedPthreadMutexUnlock), nullptr) !=
        PLTHook::SUCCESS) {
      TRACKER_ERROR("Failed to hook pthread_mutex_unlock");
    }

    if (hook_->ReplaceFunction("pthread_mutex_trylock", reinterpret_cast<void*>(&HookedPthreadMutexTrylock), nullptr) !=
        PLTHook::SUCCESS) {
